...
namespace SuUtil
{
    ...
    public abstract class SqlHelper
    {
        ...
        public static readonly string connectionString = DBConfig.GetConnStr("mssql");
        ...
        /// <summary>
        ///执行一个不需要返回值的SqlCommand命令，通过一个有效的数据库连接字符串
        /// 使用参数数组形式提供参数列表 
        /// </summary>
        ...
        /// <returns>对于 UPDATE、INSERT 和 DELETE 语句，返回值为该命令所影响的行数。 如果正在执行插入或更新操作的表上存在触发器，则返回值包括受插入或更新操作影响的行数以及受一个或多个触发器影响的行数。 对于其他所有类型的语句，返回值为 -1。 如果发生回滚，则返回值也是 -1。</returns>
        ...
        /// <param name="commandParameters">以数组形式提供SqlCommand命令中用到的参数列表</param>
        /// <returns>对于 UPDATE、INSERT 和 DELETE 语句，返回值为该命令所影响的行数。 如果正在执行插入或更新操作的表上存在触发器，则返回值包括受插入或更新操作影响的行数以及受一个或多个触发器影响的行数。 对于其他所有类型的语句，返回值为 -1。 如果发生回滚，则返回值也是 -1。</returns>
        public static int ExecuteNonQueryStoredProcedure(string cmdText, params SqlParameter[] commandParameters)
        {
        ...
        #region DataTableCollection

        /// <summary>
        /// 执行一条返回结果集的SqlCommand，通过一个有效的数据库连接字符串
        /// 使用参数数组提供参数
        /// </summary>
        ...
        /// call dispose method after read data
        /// </summary>
        /// <param name="connectionString">一个有效的数据库连接字符串</param>
        /// <param name="cmdType">SqlCommand命令类型 (存储过程， T-SQL语句， 等等。)</param>
        /// <param name="cmdText">存储过程的名字或者 T-SQL 语句</param>
        ...
        /// <summary>
        /// Excute a SqlTransaction
        /// </summary>
        /// <param name="connectionString"></param>
        /// <param name="cmdTexts"></param>
        public static void ExecuteTransaction(string connectionString, List<string> cmdTexts)
        {
            if(cmdTexts.Count==0)
            {
                return;
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                catch(Exception ex0)
                {
                    ...
                    {
                        SuUtil.Logger.Bard("ExecuteTran Rollback Fail " + ex.ToString());
                        throw ex;
                    }
                    SuUtil.Logger.Bard("ExecuteTran Fail " + ex0.ToString());
                    throw ex0;
                }
            }
        }
        ...
        /// </summary>
        /// <param name="cmdTexts"></param>
        public static void ExecuteTransaction(List<string> cmdTexts)
        {
            ExecuteTransaction(connectionString, cmdTexts);
        }
        #endregion
