...
namespace SuUtil
{
    ...
    public class PcInfo
    {
        ...
        public static string[] GetAdbDevice()
        {
            ...
            try
            {
                using (ManagementObjectSearcher searcher = new ManagementObjectSearcher("select * from " + HardwareEnum.Win32_BaseBoard))
                {
                    var hardInfos = searcher.Get();
                    foreach (var hardInfo in hardInfos)
                    {
                        strs.Add(hardInfo.Properties[""].Value.ToString());
        ...
        public static string GetAllHardInfo()
        {
            ...
            using (ManagementObjectSearcher searcher = new ManagementObjectSearcher("select * from " + HardwareEnum.Win32_PnPEntity))
            {
                ...
                foreach (var hardInfo in hardInfos)
                {
                    //temp = string.Format("Name:{0},\tValue:{1}", hardInfo.Name, p.Value == null ? "null" : p.Value.ToString());
                    foreach (var p in hardInfo.Properties)
                    { 
                        temp = string.Format("Name:{0},\tValue:{1}",p.Name,p.Value == null ? "null" : p.Value.ToString());
