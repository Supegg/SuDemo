...
namespace SuUtil
{
    public class HardwareClass
    {
        ...
        /// 
        /// 
        public static List<HardwareInfo> GetHardwareTable()
        {
            List<HardwareInfo> _ReturnList = new List<HardwareInfo>();
        ...
        /// <returns></returns>
        public static bool IsDeviceReady(string name)
        {
            foreach (var info in GetHardwareTable())
            {
                if (info.DeviceName.IndexOf(name) != -1)
                {
                    return true;
                }
            }
